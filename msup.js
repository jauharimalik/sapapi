...
const REMOTE_FOLDER = '/STG/INBOX/SUPPLIER'; // Changed for OUTLET
...
const CUSTOMER_DATA_BASE_SQL = `
    SELECT
        ...
        AD.City AS [ADDRESS2],
        '' AS [ADDRESS3], 
        ISNULL(AD.ZIPCODE, '') AS Postal_Code,
        '' AS Phone_Number,
        '' AS fax,
        '' AS Contact_Person,
        'FEFO' AS Method_pick,
...
async function generateAndUploadInsert() {
    ...
    const fileName = `CStrg_MSupRDA_Ins_${year}${month}${day}_${hours}${minutes}.txt`; 
...
async function generateAndUploadUpdate() {
    ...
    const fileName = `CStrg_MSupRDA_Upd_${year}${month}${day}_${hours}${minutes}.txt`; 
    const localFilePath = path.join(LOCAL_FOLDER, fileName);
    const remoteFilePath = `${REMOTE_FOLDER}/${fileName}`;
